...
CREATE TABLE cboard (
  ...
  cboard_photo1 VARCHAR2(12) NOT NULL, -- 사진1
  cboard_photo2 VARCHAR2(12), -- 사진2
  cboard_regdate DATE default sysdate NOT NULL, -- 게시물 등록일
  cboard_modifydate DATE, -- 게시물 최종 수정일
  cboard_ip VARCHAR2(12) NOT NULL, -- ip 주소
  ...
  CONSTRAINT cboard_fk FOREIGN KEY (order_num) REFERENCES order (order_num),
  CONSTRAINT cboard_fk FOREIGN KEY (mem_num) REFERENCES omember (mem_num)
);
...
CREATE TABLE cboard_reply (
  ...
  re_ip VARCHAR2(12) NOT NULL, -- 댓글 ip
  re_fav NUMBER(5), -- 댓글 좋아요
  cboard_num NUMBER NOT NULL, -- 게시글 번호
  mem_num NUMBER NOT NULL, -- 회원 번호
  CONSTRAINT reply_pk PRIMARY KEY (re_num),
  CONSTRAINT reply_fk FOREIGN KEY (mem_num) REFERENCES omember (mem_num),
  CONSTRAINT reply_fk FOREIGN KEY (cboard_num) REFERENCES cboard (cboard_num)
);
...
CREATE TABLE cboard_report (
  ...
  CONSTRAINT report_fk FOREIGN KEY (mem_num) REFERENCES omember (mem_num),
  CONSTRAINT report_fk FOREIGN KEY (re_num) REFERENCES cboard_reply (re_num)
);
...
CONSTRAINT fav_fk FOREIGN KEY (cboard_num) REFERENCES cboard (cboard_num), 
CONSTRAINT fav_fk FOREIGN KEY (mem_num) REFERENCES omember (mem_num) 
);
