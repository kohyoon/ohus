		...
		<ul class="detail-sub">
			<li>
				...
				<c:if test="${market.market_status == 0}">
					<c:if test="${user_num == market.mem_num}">
						<input type="button" value="채팅하기" onclick="location.href='chatroom.do?mem_num=${user_num}'">
					</c:if>
					<c:if test="${user_num != market.mem_num}">
						<input type="button" value="채팅하기" onclick="location.href='chat.do?buyer_num=${user_num}&seller_num=${market.mem_num}'">
					</c:if>
				</c:if>
				<c:if test="${market.market_status == 1}">
					<input type="button" value="채팅보기" disabled="disabled">
				</c:if>
			</li>
		</ul>
	</div>
